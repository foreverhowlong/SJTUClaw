...
课程项目 SJTUClaw:一个最小 agent runtime,需覆盖 [generalSpec.MD](generalSpec.MD) 的全部 11 项要求(LLM 调用、CLI 多轮对话、多 session、上下文管理、压缩、tool call、Gateway+图形化入口、定时任务、workspace+approval、skill system、统一 runtime),并加入两个增强点:**Agent 思考过程实时可视化** 与 **模型自主维护长期记忆**。
...
agent loop 内部每一步(LLM 流式输出、tool call、approval 等待、压缩)都 emit 结构化 `AgentEvent`;memory 修改复用普通 tool 事件:
...
`turn_start / llm_delta / llm_message / tool_call / tool_result / approval_required / approval_resolved / compaction_started / compaction_done / error / turn_end`
...
run_turn():
  ...
  5. 首个成功用户 turn 提交后,保存并行生成的简短 session 标题
```
...
### 4. 模型自主维护长期记忆
- 模型可调用 `save_memory(content)` 保存用户明确表达、跨 session 仍有价值的长期事实或偏好。保存直接落盘,不需要 approval。
- 模型可调用 `delete_memory(memory_id)` 永久删除错误或过期记忆。删除必须复用 advanced tool approval 与 execution journal。
- 两个工具都复用普通 `tool_call / tool_result` 时间线,不定义 memory 专用事件。系统不做定期 reflection、自动合并、纠错或语义去重。
- 手动管理继续使用 CLI `/memory list|add|delete` 与 Web Memory 面板,满足 spec 的手动增删查要求。
- 首个成功的非 scheduled 用户 turn 会并行生成简短 session 标题;只覆盖默认“新会话”,失败不影响正常回答。

### 5. 思考过程可视化(AgentTrace.tsx)—— 记忆点 1
WebSocket 收到 AgentEvent 流,渲染为垂直时间线:思考文本流式打字、tool call 卡片(参数+结果折叠)、approval 卡片(内嵌批准/拒绝按钮)、压缩提示。核心工作量在前端渲染,后端零额外成本。
...
9. **M9 体验收尾**:save/delete memory tools + 首轮 session 标题 + 可视化打磨 + 演示脚本
